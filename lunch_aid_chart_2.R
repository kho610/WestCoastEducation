...
chart_2_data <- edu_demo_data %>%
  ...
  summarise(teachers = mean(teachers, na.rm = T),
            ...
            free_lunch = mean(free_lunch, na.rm = T),
            reduced_lunch = mean(reduced_lunch, na.rm = T),
            ratio = mean(student_teacher_ratio, na.rm = T)) %>%
  filter(Locale != "N") %>%
  select(teachers, students, ratio, Locale, free_lunch, reduced_lunch)

# scatter plot

ggplot_scatter <- function(df){
  ggplot(data = df) +
    geom_point(mapping = aes(x = students, y = free_lunch, color = Locale)) +
    scale_x_continuous() +
    scale_y_continuous() +
    scale_color_discrete() +
    labs(title = "Number of Students vs. Free Lunches",
         y = "Free Lunch (Average)",
